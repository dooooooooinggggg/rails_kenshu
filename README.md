...
railsでは、サーバー側の変更を行った際、保存したのに、反映されないことがあります。
例えば、新しいルーティングを作成した時、そこのにアクセスすると、「no routes match」みたいなエラーが出ます。
こういうときは、サーバーを再起動してください。コントロールCで終了して、またrails s -b 0.0.0.0を叩けば平気です。
...
cakephpの研修でやったことは、わざわざここでやるほどのことでもないかと思うので、
今回は、jqueryなどの技術には触れないようにします。

railsでは、ほとんどの機能(ログインなど)は、ライブラリで提供されています。
なので、実際習得すべきことは、それらのライブラリをどう扱うか、や、ルーティングをどういい感じに書くか、いかにシンプルに書くか、などです。
